...
function classify(witnessPath,sourcePath){
 ...
 const registry=JSON.parse(fs.readFileSync(path.join(__dirname,'segmentos-conectados-b9f1a7b93ca1.json'),'utf8'));
 const level=w.level,spec=registry.levels.find(l=>l.level===level);assert.ok(spec,'Level must be one-based');
 const compatibilityPath=path.join(__dirname,'compatibilidad-registro-round11.json');
 const compatibility=fs.existsSync(compatibilityPath)?JSON.parse(fs.readFileSync(compatibilityPath,'utf8')):null;
 const auditedCompatible=compatibility?.sourceHash===sha(source)&&compatibility.registryHash===sha(fs.readFileSync(path.join(__dirname,'segmentos-conectados-b9f1a7b93ca1.json')))&&compatibility.levels.includes(level);
 assert.ok(sha(source)===registry.sourceHashes[spec.source]||auditedCompatible,'Use preregistered or explicitly audited compatible source; no silent acceptance.');
 ...
 for(const c of commands){assert.ok(Array.isArray(c.a)&&c.a.length===3&&[-1,0,1].includes(c.a[0])&&[0,1].includes(+c.a[1])&&[0,1].includes(+c.a[2]),'Only legal movement/jump/down tuples');assert.ok(Number.isInteger(c.frames)&&c.frames>0,'Positive integer input duration');}
 ...
 function contrast(s,e){
  ...
  // An entry commanded backwards cannot establish necessity for forward travel.
  const incomplete=(requested>600&&!dead&&!crossed)||e.direction!==1;
 ...
 assert.equal(frame,w.frames??w.metrics.frames,'Normalized replay must preserve declared full-route frames');
 assert.equal(Object.values(contexts).reduce((a,b)=>a+b,0),frame,'Every simulation frame has exactly one context category');
