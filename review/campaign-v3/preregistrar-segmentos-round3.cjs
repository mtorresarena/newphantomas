...
const src={base:fs.readFileSync(path.join(__dirname,'index-base-31d4610.html'),'utf8'),candidate:fs.readFileSync(path.join(__dirname,'round3/index.html'),'utf8')};
...
const output=JSON.stringify(manifest,null,2)+'\n';fs.writeFileSync(path.join(__dirname,'segmentos-preregistrados-round3.json'),output);console.log(JSON.stringify({file:'segmentos-preregistrados-round3.json',sha256:hash(output),levels:levels.length,simulationSteps:0}));
