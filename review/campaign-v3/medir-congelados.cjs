...
const {classify}=require('./clasificador-comun.cjs'),createGame=require('./route-harness-score1.cjs'),sha=s=>crypto.createHash('sha256').update(s).digest('hex');
const frozen=path.join(__dirname,'harmonized-v2'),output=path.join(__dirname,'harmonized-classification');fs.mkdirSync(output,{recursive:true});
...
const classifierHash=sha(fs.readFileSync(path.join(__dirname,'clasificador-comun.cjs'))),results=[];
