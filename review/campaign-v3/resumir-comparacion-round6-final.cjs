...
const root=path.join(__dirname,'round6-classification'),frozen=path.join(__dirname,'round6-review-frozen'),manifest=JSON.parse(fs.readFileSync(path.join(frozen,'manifest.json'),'utf8'));assert.equal(manifest.complete,true);assert.equal(manifest.files.length,24);
