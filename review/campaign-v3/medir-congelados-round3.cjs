...
const {classify}=require('./clasificador-comun-round3.cjs'),createGame=require('./route-harness-score1.cjs'),sha=s=>crypto.createHash('sha256').update(s).digest('hex');
const frozen=path.join(__dirname,'round3-review-frozen'),output=path.join(__dirname,'round3-classification');fs.mkdirSync(output,{recursive:true});
...
const classifierHash=sha(fs.readFileSync(path.join(__dirname,'clasificador-comun-round3.cjs'))),results=[];
...
for(const entry of manifest.files){
 ...
 const record={policy:entry.policy,protocol:w.protocol,budget:w.budget,classifierHash,normalization:{originalChunks:chunks.length,coalescedChunks:coalesced.length,fullEndpointIdentical:true,frames:originalEnd.frames},result};
