...
function classify(witnessPath,sourcePath){
 ...
 const auditedCompatible=compatibility?.sourceHash===sha(source)&&compatibility.registryHash===sha(fs.readFileSync(path.join(__dirname,'segmentos-preregistrados.json')))&&compatibility.levels.includes(level);
