...
for(let frame=0;frame<2400&&!api.player.dead&&!api.boss.defeated;frame++){
 ...
 else if(p.onGround&&!prevJump&&p.y+9>t.y+10){jump=true;hold=20;}
 else if(p.onGround&&!prevJump&&p.y+9<t.y-15){jump=true;down=true;}
 route.input([dir,+jump,+down]);prevJump=jump;route.step();inputs.push([dir,+jump,+down]);
 if(b.hp!==previousHp){events.push({frame:frame+1,hp:b.hp,conductor:id,x:p.x,y:p.y,energy:p.energy});previousHp=b.hp;}
}
...
fs.writeFileSync(path.join(__dirname,'probe-jefe-base.json'),JSON.stringify(result,null,2)+'\n');console.log(JSON.stringify({...result,inputs:inputs.length},null,2));
