...
function classify(witnessPath,sourcePath){
 ...
 const registry=JSON.parse(fs.readFileSync(path.join(__dirname,'segmentos-preregistrados-round6.json'),'utf8'));
 const level=w.level,spec=registry.levels.find(l=>l.level===level);assert.ok(spec,'Level must be one-based');
 const compatibilityPath=path.join(__dirname,'compatibilidad-fuente-round6.json');
 const compatibility=fs.existsSync(compatibilityPath)?JSON.parse(fs.readFileSync(compatibilityPath,'utf8')):null;
 const auditedCompatible=compatibility?.sourceHash===sha(source)&&compatibility.registryHash===sha(fs.readFileSync(path.join(__dirname,'segmentos-preregistrados-round6.json')))&&compatibility.levels.includes(level);
