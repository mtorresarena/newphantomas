...
const targets=[...[['3-1','charge'],['4-2','charge'],['5-1','charge'],['5-2','weight']].map(([id,kind])=>({level:5,id,kind})),...[['3-1','mirror'],['4-2','mirror'],['5-1','mirror'],['5-2','mirror'],['3-2','timed'],['4-1','timed']].map(([id,kind])=>({level:7,id,kind}))];
...
fs.writeFileSync(path.join(__dirname,'variantes-legales-round6.json'),JSON.stringify({sourceHash:sha(source),fixture:false,scope:'50 branches over ten changed variants. Every branch replays legal controls from the natural start to the recorded first approach/use proximity, then changes input only for up to900frames. No teleport, restore, direct death, healing or enemy deletion. New timed layouts 3/4 are reached in N7; this is not an N6 full-route proof.',runs},null,2)+'\n');
