...
const source=fs.readFileSync(path.join(__dirname,'round4/index.html'),'utf8'),{api}=game(source),results=[];
for(const level of [5,6,7,8]){api.newGame();api.loadLevel(level-1);const raw=api.buildLevel(api.LEVELS[level-1]);const entries=[];
 for(const t of api.campaign.trials){const origin=t.gateX-28*16,c0=origin/16,c1=t.gateX/16+1;
  const topology={kind:t.kind,duration:t.duration??null,order:t.order??null,nodes:t.nodes.map(n=>({id:n.id,role:n.role,x:n.x-t.gateX,y:n.y,initialAngle:n.initialAngle??null})),map:raw.map.map(row=>row.slice(c0,c1).join(''))};
