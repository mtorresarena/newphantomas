...
const root=path.join(__dirname,'harmonized-classification'),frozen=path.join(__dirname,'harmonized-v2'),manifest=JSON.parse(fs.readFileSync(path.join(frozen,'manifest.json'),'utf8'));assert.equal(manifest.complete,true);assert.equal(manifest.files.length,24);
