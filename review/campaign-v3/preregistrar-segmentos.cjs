...
const src={base:fs.readFileSync(path.join(__dirname,'index-base-31d4610.html'),'utf8'),candidate:fs.readFileSync(path.join(__dirname,'candidate-score1.html'),'utf8')};
...
const manifest={version:1,registered:'2026-09-08',protocol:'PREREGISTRO-ACTIVIDAD-RONDA2.md',sourceHashes:{base:hash(src.base),candidate:hash(src.candidate)},scope:'Static candidate episodes; no route read, no simulation step, no measured meaningful-action counts.',levels};
const output=JSON.stringify(manifest,null,2)+'\n';fs.writeFileSync(path.join(__dirname,'segmentos-preregistrados.json'),output);console.log(JSON.stringify({file:'segmentos-preregistrados.json',sha256:hash(output),levels:levels.length,simulationSteps:0}));
