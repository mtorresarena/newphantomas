...
const {classify}=require('./clasificador-comun-round4.cjs'),createGame=require('./route-harness-score1.cjs'),sha=s=>crypto.createHash('sha256').update(s).digest('hex');
const frozen=path.join(__dirname,'round4'),output=path.join(__dirname,'round4-classification');fs.mkdirSync(output,{recursive:true});
...
const classifierHash=sha(fs.readFileSync(path.join(__dirname,'clasificador-comun-round4.cjs'))),results=[];
const evaluatedAPI=createGame(source).api;
const reusedSources=['harmonized-v2/index.html','round3/index.html'].map(name=>{const text=fs.readFileSync(path.join(__dirname,name),'utf8');return {hash:sha(text),api:createGame(text).api};});
