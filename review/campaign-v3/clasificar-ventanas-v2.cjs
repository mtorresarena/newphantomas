...
const root=path.join(__dirname,'round4'),out=path.join(__dirname,'semantic-windows-v2-round4');fs.mkdirSync(out,{recursive:true});const source=fs.readFileSync(path.join(root,'index.html'),'utf8'),manifest=JSON.parse(fs.readFileSync(path.join(root,'manifest.json'))),registry=JSON.parse(fs.readFileSync(path.join(__dirname,'segmentos-preregistrados-round4.json')));assert.equal(sha(source),manifest.sourceHash);const observerHash=sha(fs.readFileSync(__filename)),protocolHash=sha(fs.readFileSync(path.join(__dirname,'PROTOCOLO-SEMANTICO-VENTANAS-V2.md'))),summaries=[];
for(const entry of manifest.files){const file=path.join(out,`n${entry.level}-${entry.policy}.json`),raw=fs.readFileSync(path.join(root,entry.name)),w=JSON.parse(raw);assert.equal(sha(raw),entry.hash);
 ...
 const measured=JSON.parse(fs.readFileSync(path.join(__dirname,'round4-classification',`n${entry.level}-${entry.policy}.json`))).result;assert.equal(measured.witnessHash,entry.hash);const spec=registry.levels.find(l=>l.level===entry.level),g=game(source),{api,route}=g;api.newGame();api.loadLevel(entry.level-1);api.startLevel();
