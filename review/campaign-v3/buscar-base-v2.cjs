...
const resumeFile=process.argv[4],outputTag=process.argv[5]||'v2';
...
function hash(s){const p=s.player,b=s.boss;return [Math.round(p.x/5),Math.round(p.y/4),Math.round(p.vx),Math.round(p.vy),+p.onGround,+p.jHeld,Math.floor(s.simF%400/30),p.keys,Math.floor(p.energy/15),s.items.filter(i=>i.t==='k').length,b?.state,Math.floor((b?.timer||0)/5),b?.conductors.map(c=>+c.charged).join(''),Math.round((b?.targetX||0)/10),Math.round((b?.targetY||0)/10)].join(',');}
...
for(const t of targets){
 ...
 while(heap.a.length&&expanded<limit){
  ...
  for(let ai=0;ai<actions.length;ai++){
   ...
   node.score=d+g*.07+(100-p.energy)*5;heap.push(node);
  }
  if(found)break;
 }
...
fs.writeFileSync(path.join(__dirname,`ruta-base-${outputTag}-n${level}.json`),JSON.stringify(result,null,2)+'\n');console.log(JSON.stringify({complete:result.complete,metrics}));
