...
const {api,route}=createGame(fs.readFileSync('review/campaign-v3/round6/index.html','utf8')),cases=[];
...
test('Every mirror layout connects only with the intended beam turns',()=>{
 let count=0;for(let l=4;l<8;l++){load(l);for(const t of api.campaign.trials.filter(t=>t.kind==='mirror')){assert.equal(api.mirrorTrace(t),false);const solution=require('../../tools/solve-mirror-layout')(t,(c,r)=>api.isSolid(api.tileAt(c,r)));for(const n of t.nodes.filter(n=>n.role==='mirror')){const target=solution[n.id];if(n.angle!==target)use(n);}assert.equal(api.mirrorTrace(t),true,t.id);for(let f=0;f<31;f++)api.updateCampaign();assert.equal(t.solved,true);count++;}}assert.ok(count>=7);
});
...
fs.writeFileSync('review/campaign-v3/round6-rules14.json',JSON.stringify({scope:'Isolated state fixtures, not full input routes',passed:cases.length,cases},null,2));
