...
const hash=crypto.createHash('sha256').update(fs.readFileSync('review/campaign-v3/round6-review-frozen/index.html')).digest('hex');
const runs=[];
for(let li=4;li<8;li++){
 const witness=JSON.parse(fs.readFileSync(`review/campaign-v3/round6-review-frozen/route-n${li+1}.json`));assert.equal(witness.complete,true);
 const attempts=[];
 for(let attempt=0;attempt<3;attempt++){
  const {api,route}=createGame(fs.readFileSync('review/campaign-v3/round6-review-frozen/index.html','utf8'));api.newGame();api.loadLevel(li);api.startLevel();
...
const {api,route}=createGame(fs.readFileSync('review/campaign-v3/round6-review-frozen/index.html','utf8')),base=JSON.parse(fs.readFileSync('review/campaign-v3/ruta-base-v3-n4.json'));
...
for(let li=4;li<8;li++){
 ...
 const witness=JSON.parse(fs.readFileSync(`review/campaign-v3/round6-review-frozen/route-n${li+1}.json`));
...
fs.writeFileSync('review/campaign-v3/round6-connected-replays-independent.json',JSON.stringify({sourceHash:hash,scope:'Production loop, actual movement inputs; three deterministic repetitions are not three players. Frame categories are not human decisions.',runs,transitions,finalTitle:true},null,2));
