...
const sha=s=>crypto.createHash('sha256').update(s).digest('hex'),root=path.join(__dirname,'round4'),out=path.join(__dirname,'semantic-round4-n5n6');fs.mkdirSync(out,{recursive:true});
...
const registry=JSON.parse(fs.readFileSync(path.join(__dirname,'segmentos-preregistrados-round4.json'))),categories=['mandatory_action','simple_transit','necessary_mechanism_wait','optional','repetition'];
...
for(const entry of manifest.files.filter(e=>[5,6].includes(e.level))){
 ...
 const measured=JSON.parse(fs.readFileSync(path.join(__dirname,'round4-classification',`n${entry.level}-${entry.policy}.json`))).result;assert.equal(measured.witnessHash,entry.hash);
