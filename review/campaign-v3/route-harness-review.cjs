...
module.exports=function createGame(source){
 ...
 vm.runInContext(js+`
 window.route={
  save(){return structuredClone({player,boss,ents,orbs,balls,movers,crumbles,items,check,stats,total,cam,frame,tick,simF,state,continued,alarmT,doorCd,shake,zoneName,msg,fadeT,map:L.map});},
  restore(s){({player,boss,ents,orbs,balls,movers,crumbles,items,check,stats,total,cam,frame,tick,simF,state,continued,alarmT,doorCd,shake,zoneName,msg,fadeT}=structuredClone(s));L.map=s.map.map(r=>r.slice());parts=[];},
