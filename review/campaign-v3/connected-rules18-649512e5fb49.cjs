...
const assert=require('assert/strict'),fs=require('fs'),createGame=require('./route-harness-score1.cjs');
const {api,route}=createGame(fs.readFileSync('review/campaign-v3/connected-inspection-649512e5fb49.html','utf8')),cases=[];
...
test('Every mirror layout connects only with the intended beam turns',()=>{
 let count=0;for(let l=4;l<8;l++){load(l);for(const t of api.campaign.trials.filter(t=>t.kind==='mirror')){assert.equal(api.mirrorTrace(t),false);const solution=require('../../tools/solve-mirror-layout')(t,(c,r)=>api.isSolid(api.tileAt(c,r)));for(const n of t.nodes.filter(n=>n.role==='mirror')){const target=solution[n.id];if(n.angle!==target)use(n);}if(t.dualLight){for(let f=0;f<31;f++)api.updateCampaign();assert.equal(t.primed,true);assert.equal(t.solved,false);use(t.nodes.find(n=>n.role==='prism'));}assert.equal(api.mirrorTrace(t),true,t.id);for(let f=0;f<31;f++)api.updateCampaign();assert.equal(t.solved,true);count++;}}assert.ok(count>=7);
});
...
fs.writeFileSync('review/campaign-v3/connected-rules18-649512e5fb49.json',JSON.stringify({sourceHash:require('crypto').createHash('sha256').update(fs.readFileSync('review/campaign-v3/connected-inspection-649512e5fb49.html')).digest('hex'),scope:'Isolated state fixtures, not full input routes',passed:cases.length,cases},null,2));
