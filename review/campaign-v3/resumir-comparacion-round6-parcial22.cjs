...
const root=path.join(__dirname,'round6-classification'),frozen=path.join(__dirname,'round6-review-frozen'),manifest=JSON.parse(fs.readFileSync(path.join(frozen,'manifest.json'),'utf8'));assert.ok(manifest.files.length >= 12);
...
const report={sourceHash:manifest.sourceHash,scope:'Partial frozen source6 set; all12 references present but two campaign policies are still missing. Conservative interval comparison; no grade or human-duration inference. A failed single policy is distinguished from failing every comparable policy.',rows,policies,uniqueness};
fs.writeFileSync(path.join(__dirname,'comparacion-round6-parcial-22.json'),JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify({policies,uniqueness,riskN4speed:rows.find(r=>r.level===4&&r.policy==='speed')},null,2));
