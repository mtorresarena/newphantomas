...
module.exports=function createGame(source){
 ...
 const sandbox={console,Math,structuredClone,setInterval:()=>0,clearInterval:noop,setTimeout:()=>0,clearTimeout:noop,
  ...
  addEventListener:(type,fn)=>{if(!listeners.has(type))listeners.set(type,[]);listeners.get(type).push(fn);},navigator:{},localStorage:{getItem:()=>null,setItem:noop},
 ...
 vm.runInContext(js+`
 const hudCalls=[];const originalText=text;text=function(s,...args){hudCalls.push({s,args});};
 window.route={ hudProbe(){hudCalls.length=0;drawCampaignHUD();return structuredClone(hudCalls);},
 ...
 sandbox.route.press=(code,advance=true)=>{const event={code,key:code,repeat:false,preventDefault:noop};for(const fn of listeners.get('keydown')||[])fn(event);if(advance)sandbox.route.step();for(const fn of listeners.get('keyup')||[])fn(event);};
