'use strict';
const assert=require('assert/strict'),fs=require('fs'),crypto=require('crypto'),createGame=require('./route-harness');
const hash=crypto.createHash('sha256').update(fs.readFileSync('index.html')).digest('hex');
const runs=[];
for(let li=4;li<8;li++){
 const witness=JSON.parse(fs.readFileSync(`review/campaign-v3/route-n${li+1}.json`));assert.equal(witness.complete,true);
 const attempts=[];
 for(let attempt=0;attempt<3;attempt++){
  const {api,route}=createGame();api.newGame();api.loadLevel(li);api.startLevel();
...
const {api,route}=createGame(),base=JSON.parse(fs.readFileSync('review/campaign-v3/ruta-base-v3-n4.json'));
...
for(let li=4;li<8;li++){
 ...
 const witness=JSON.parse(fs.readFileSync(`review/campaign-v3/route-n${li+1}.json`));
...
fs.writeFileSync('review/campaign-v3/current-replays.json',JSON.stringify({sourceHash:hash,scope:'Production loop, actual movement inputs; three deterministic repetitions are not three players. Frame categories are not human decisions.',runs,transitions,finalTitle:true},null,2));
